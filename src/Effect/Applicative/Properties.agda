...
module _ 
  ...
  private
    module ≈-Reasoning {A : Set ℓ} =
      Reasoning (record { isEquivalence = isEquivalence {A} })

  module properties (applicative : Applicative F _≈_) where
  ...
  -}
  record BareApplicative : Set (suc (ℓ ⊔ ℓ′)) where
    infixl 4 _<*>_
  ...
  mkApplicative : BareApplicative → Applicative F _≈_
  mkApplicative bare = record { rawApplicative = raw; isApplicative = record {isApplicativeImpl} }
    where
      open IsEquivalence {{...}}
